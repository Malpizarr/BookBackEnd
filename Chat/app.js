...
wss.on("listening", () => {
  console.log(
    `Servidor iniciado y escuchando en el puerto ${process.env.PORT || 8083}`,
  );
});
...
wss.on("connection", function connection(ws, req) {
  ...
  ws.on("message", async function incoming(message) {
    ...
    if (msg.type === "friendsListRequest") {
      ...
      if (friendsList) {
        ...
        ws.send(
          JSON.stringify({ type: "friendsList", friends: onlineFriends }),
        );
      }
    } else {
      try {
      ...
      if (msg.type === "chatRequest") {
        getChatMessages(userId, msg.friendId, (error, chatMessages) => {
          ...
          ws.send(
            JSON.stringify({ type: "chatMessages", messages: chatMessages }),
          );
        });
      }
      ...
      if (msg.type === "friendRequest") {
        ...
        if (friendWs && friendWs.readyState === WebSocket.OPEN) {
          ...
          console.log(
            `WebSocket para el amigo con ID ${msg.friendId} no está disponible o abierto.`,
          );
        }
      }
  ...
  async function getFriendsList(userId, token) {
    ...
    try {
      const response = await axios.get(
        process.env.PRODUCTION_URL_FRIENDS + `/api/friendships/friends`,
        {
      ...
      friendsListCache[userId] = response.data.map(
  ...
  setInterval(
    ...
    1000 * 60 * 60,
  ); // Limpia el caché cada hora, ajustar según necesidad
...
function sendMessage(senderId, receiverId, messageContent) {
  ...
  const insertQuery =
    "INSERT INTO messages (senderId, receiverId, message, timestamp, is_read) VALUES (?, ?, ?, ?, FALSE)";
  pool.query(
    insertQuery,
    [senderId, receiverId, messageContent, timestampMySQL],
